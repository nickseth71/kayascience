...
class CartItems extends HTMLElement {
  ...
  updateQuantity(line, quantity, name, variantId) {
    document.querySelectorAll(".cart-items").forEach(element => {
        element.classList.add("disabled");
    });
    console.log("updateproduct");
    const cartTable = document.querySelector('.template__cart table.cart-items');

    if (cartTable) {
      cartTable.style.pointerEvents = 'none';
      cartTable.style.opacity = '0.6';
    }
    ...
    fetch(`${routes.cart_change_url}`, { ...fetchConfig(), ...{ body } })
      ...
      .then((state) => {
        ...
        publish(PUB_SUB_EVENTS.cartUpdate, {
          ...
          setTimeout(function() {
          $('.cart__checkout-button').attr('disabled',false);
        },5000)
         
        window.cartFreeProduct();
        window.gift_for_all_product();
        window.b2g1gift_product();
        setTimeout(function(){
            document.querySelectorAll(".cart-items").forEach(element => {
